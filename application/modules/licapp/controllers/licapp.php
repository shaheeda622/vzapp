 ...
 */
class licapp extends MX_Controller{
  ...
  public function initform(){
    ...
    if(!$la_name){
      $row = $this->db->select('MAX(la_name) as pk', FALSE)->get('licenseapp')->row();
      $la_name = (($row && $row->pk) ? $row->pk : '');
      ...
      $this->db->insert('licenseapp', $licence_app);
      foreach($shareholders as $shareholder){
        $shareholder['sh_license_name'] = $la_name;
        $this->db->insert('shareholder', $shareholder);
      }
    }
    else{
      if($this->db->where('la_name', $la_name)->count_all_results('licenseapp') > 0){
        $this->db->where('la_name', $la_name)->update('licenseapp', $licence_app);
      }
      else{
        $this->db->insert('licenseapp', $licence_app);
      }
      $this->db->where('sh_license_name', $la_name)->delete('shareholder');
      foreach($shareholders as $shareholder){
        $shareholder['sh_license_name'] = $la_name;
        $this->db->insert('shareholder', $shareholder);
      }
